...
NUCLEAR_CENTER_SITES=\$(mktemp)

join <(sort "$NUCLEAR_CHR") "$MAPPABLE_REGIONS" > "\$NUCLEAR_MAPPABLE_REGIONS"
join <(sort "$NUCLEAR_CHR") "$CHROM_SIZES" > "\$NUCLEAR_CHROM_SIZES"
join <(sort "$NUCLEAR_CHR") <(unstarch "$CENTER_SITES") | starch --gzip - > "\$NUCLEAR_CENTER_SITES"

"$HOTSPOT_SCRIPT"  -F 0.5 -p varWidth_20_${LIBRARY_NAME} -M "\$NUCLEAR_MAPPABLE_REGIONS" -c "\$NUCLEAR_CHROM_SIZES" -C "\$NUCLEAR_CENTER_SITES" "$FINAL_UNIQUES_BAM"  "$HOTSPOT2_DIR"
