...
def helpMessage() {
  log.info"""
    Usage: nextflow run process_bwa_paired.nf \\
             --r1 r1.fastq.gz \\
             --r2 r2.fastq.gz \\
    ...
    --read_length [length] How long our reads are                                          (36)
    """.stripIndent();
}
...
/*
 * Step 1: Trim to the appropriate length
 */
process trim_to_length {
  ...
  if (params.trim_to != 0)
    ...
    """
    zcat $split_r1 | awk 'NR%2==0 {print substr(\$0, 1, $params.trim_to)} NR%2!=0' | gzip -c -1 > r1.trim.fastq.gz
    zcat $split_r2 | awk 'NR%2==0 {print substr(\$0, 1, $params.trim_to)} NR%2!=0' | gzip -c -1 > r2.trim.fastq.gz
    """
  else
    """
    ln -s $split_r1 r1.trim.fastq.gz
    ln -s $split_r2 r2.trim.fastq.gz
    """
