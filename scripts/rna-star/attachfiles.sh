...
$ATTACH_ALIGNMENT --attach_file trimmed/${FLOWCELL}_${SAMPLE_NAME}_R1.fastq.gz --attach_file_purpose r1-fastq-trimmed --attach_file_type gzipped-fastq
if [[ -n "$PAIRED" ]] ; then
  $ATTACH_ALIGNMENT --attach_file trimmed/${FLOWCELL}_${SAMPLE_NAME}_R2.fastq.gz --attach_file_purpose r2-fastq-trimmed --attach_file_type gzipped-fastq
fi
